...
const totalScrollWidth = (CONFIG.slideCount - 1) * CONFIG.spacingX;
...
const Scene = ({ activeIndex, setActiveIndex, progressRef }) => {
  ...
  useFrame(() => {
    ...
    const targetScroll = rawProgress * totalScrollWidth;
    currentScroll.current += (targetScroll - currentScroll.current) * CONFIG.lerpSpeed;

    const rawIndex = Math.round(currentScroll.current / CONFIG.spacingX);
    ...
    camera.lookAt(xMove + 10, 0, -zMove - 20);
  ...
  return (
    <section ref={componentRef} className="relative z-20 bg-transparent" id="projects-3d">
      ...
      <div ref={pinRef} className="hidden md:flex relative w-full h-screen overflow-hidden flex-row">
          ...
          <div
            className={`
                relative z-10 w-[40%] h-full flex flex-col justify-center p-16 lg:p-24 border-r transition-colors duration-500
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="relative z-10">
                            ...
                            <h2 className={`text-5xl lg:text-7xl font-sans font-thin italic mb-6 leading-tight tracking-tight ${localIsLight ? "text-black drop-shadow-none" : "text-white drop-shadow-lg"}`}>
                                {activeProject.title}
                            </h2>
